...
- **「切換顯示」欄位標籤用跟清單項目相同的 flex 結構（留白 `.name-wrap` + 文字 + 固定寬度 spacer）疊出對齊效果**，而不是直接用 `text-align: right` 之類的簡便寫法——因為要對齊的是「開關」這個中間欄位，不是最右側，跟清單項目本身共用同一個排版骨架，欄位寬度變化時（例如以後名稱欄位寬度調整）標籤還是會跟著開關走，不會各自維護一份寬度數字。`.store-manage-header-spacer` 的 56px 是估算值，抓的是「刪除」按鈕的渲染寬度，需求文件裡也明講對不齊可以之後再微調。
- **這次能確認 GAS 已經是使用者重新部署過的新版**：驗證時發現先前測試留下的「小北百貨」「大東百貨」「PW測試賣場」x2 都不見了、只剩一筆「測試賣場」（隱藏中）——這代表 `deleteCategory` 這個新 action 已經生效，使用者应该已經自行用新的刪除按鈕清理過舊測試資料。剩下這筆「測試賣場」不確定是不是使用者自己在測試新功能留下的，這次沒有動它，留給使用者自行決定要不要刪除。
- **已用 Playwright 驗證**：摺疊標題文字改成「賣場管理」；「切換顯示」標籤大致對齊在開關欄位正上方；截圖確認各列排版整齊、開關跟刪除鈕貼齊右側；新增、切換顯示/隱藏功能都做了回歸測試，正常運作。這批純前端排版調整，不涉及 GAS。
